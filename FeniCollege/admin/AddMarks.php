...
<input type="text" name="to" id="to" value="<?php  echo $toss;?>"  style="width:120px; background:#F7071D; border:none; font-weight:bold; color:#fff; height:25px;  text-align:center;" onchange="return showChekTo()"  onkeyup="return showChekTo()"/>
	...
	AND `running_student_info`.`group_id`='$groupname[0]' AND `subject_registration_table`.`subject_id`='$sub_name' AND `subject_information`.`examID`='$examtype[0]' AND `subject_information`.`subjectId`='$sub_name'  AND `student_acadamic_information`.`session2`='$session' AND `running_student_info`.`class_roll` BETWEEN $fromss AND $toss GROUP BY `running_student_info`.`student_id`  ORDER BY `running_student_info`.`class_roll` ASC";
//print $showAll;
