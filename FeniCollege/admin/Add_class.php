...
if(isset($_POST['add']))
	{
		...
		{
			$query="INSERT INTO `add_class` (`id`,`class_section_id`,`class_name`) VALUES ('".$fetch[0]."','$exploide[1]','$classname')";
...
if(isset($_POST['Update']))
	{
		...
		if(!empty($id) && !empty($classname) && $class_section!="Select One")
		{
			$query="REPLACE INTO `add_class`(`id`,`class_section_id`,`class_name`) VALUES ('$id','$exploide[1]','$classname')";
...
//view data...............................	
	if(isset($_POST['View']))
	{
        ...
        "<td colspan='2' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>View All Class</span>"."</td>"."</tr>";
		$select_alll="SELECT `add_class_section`.`Class_Section`,`add_class`.* FROM `add_class` INNER JOIN `add_class_section` ON `add_class`.`class_section_id`=`add_class_section`.`id` ORDER BY `add_class`.`class_name` ASC";
		$chek_all=$db->select_query($select_alll);
		if($chek_all){
		$table.="<tr>"."<td>Class Section Name</td>"."<td>Class  Name</td>"."<td>Edit</td>"."</tr>";
		while($a=$chek_all->fetch_array())
		{
			$table.="<tr class='info'>";
			$table.="<td>".$a[0]."</td>"."<td>".$a[3]."</td>";
			$table.="<td>";
				...
				<td class="info">
					...
					<input type="hidden" readonly=""  name="id" value="<?php echo $fetch[0];?>" />
						<select name="class_section" class="form-control">
								...
								<?php if($chek){?>
									<option value="<?php echo "$fetch[4]and$fetch[3]"?>"><?php echo $fetch[4];?></option>
								<?php } ?>
							<?php 
