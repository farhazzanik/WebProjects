...
//view data...............................	
	if(isset($_POST['View']))
	{
		...
		if($result)
		{
			$select_class="SELECT `exam_type_info`.*,`add_class`.`class_name` FROM `exam_type_info` INNER JOIN `add_class` ON `add_class`.`id`=`exam_type_info`.`select_class` GROUP BY `exam_type_info`.`select_class` ORDER BY `add_class`.`class_name` ASC";
        ...
        $table.="<tr class='warning' >"."<td align='left' colspan='3'>"."<a href='Addexamtype.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."<span class='text-success' style='font-size:18px; padding-left:380px; font-weight:blod;'>পরীক্ষা  সমূহ</span>"."</td>"."</tr>";
