...
error_reporting(1);
    ...
    global $table;
    global $chek_query;
$fetch_query[0]=$db->withoutPrefix('routine','Routine_id',"19".$prefix,'12');
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="INSERT INTO `routine` (`Routine_id`,`Routine_type`,`Routine_title`,`Class`,`Year`,`Extension`,`Description`) VALUES('".$fetch_query[0]."','$type','$title','$selection','$year','$file_ext','$detaile')";
               // print_r($inserquery);
...
if(isset($_POST["view"]))
{
    $query="select * from routine ORDER BY `Routine_id` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='Class_Routin.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>Class Routine </span>"."</td>"."</tr>";
        if($result){
            $table.="<tr>"."<td>Type</td>"."<td>Title</td>"."<td>Class</td>"."<td>Year</td>"."<td>Details Or File</td>"."<td>Edit Or Delete</td>"."</tr>";
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `routine` WHERE `Routine_id`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='Class_Routin.php'</script>";
    $fetch_query[0]=$db->withoutPrefix('routine','Routine_id',"19".$prefix,'12');
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM `routine` WHERE `Routine_id`='".$_GET["edit"]."'";
    ...
    {   
       $type=$db->escape($_POST["type"]);
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="replace INTO `routine` (`Routine_id`,`Routine_type`,`Routine_title`,`Class`,`Year`,`Extension`,`Description`) VALUES('".$_POST["id"]."','$type','$title','$selection','$year','$file_ext','$detaile')";
               // print_r($inserquery);
...
<html lang="en">
 ...
 function checkforText()
    ...
    <?php if(isset($_POST["view"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Record  Found"."<a href='Class_Routin.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
...
Class Routine </span> </td>
        ...
        <td>
          <div class="col-sm-12 col-md-12">
                    ...
                    ?>
                         <option>Joint routine</option>
                        <option>Class Wise Routine </option>
                        <option>Teacher Wise Routine </option>
                    </select>
          </div>
          </td>
      </tr>
          ...
          <div class="col-sm-12 col-md-12">
            <select name="selection" class="form-control">
                   ...
                   <?php 
								...
								{
							?>
							<option><?php echo $fetchsection[2];?></option>
							<?php } } ?>
            </select>
          </div>
          </td>
        ...
        <td>
          <div class="col-sm-12 col-md-12">
            ...
            <?php
                        if($chek_query){
                            ?>
                            <option><?php echo $fetch_query[4]; ?></option>
                            <?php
                        }
                    ?>
                    <?php
...
Details &nbsp;</td>
        <td>:</td>
         <td>
            <div class="col-sm-12 col-md-12">
            <textarea  name="descreption" class="form-control" disabled="disabled"  id="redactor"><?php if($chek_query){echo $fetch_query[6]; } else { echo ""; }?> </textarea>
            </div>
            </td>
