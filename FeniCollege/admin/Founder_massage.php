<?php
 ...
 $select_all="select * from founder_message";
 ...
 if($chek_all)
 {
    ...
    {   
        ...
        if(!empty($_POST["name"])  &&  !empty($_POST["title"]) && !empty($detaile))
         ...
         {
                                    @$inserquery="INSERT INTO `founder_message` (`id`,`name`,`title`,`details`) VALUES('1','$name','$title','$detaile')";
...
{
    $Delete="DELETE FROM `founder_message` WHERE `id`='1'";
    $db->delete_query($Delete);
    print "<script>location='Founder_massage.php'</script>";
    ...
    {   
        ...
        if(!empty($_POST["name"])  &&  !empty($_POST["title"]) && !empty($detaile))
         ...
         {
                                    @$inserquery="replace INTO `founder_message` (`id`,`name`,`title`,`details`) VALUES('1','$name','$title','$detaile')";
    ...
    <form name="notice" action="" method="post"  enctype="multipart/form-data" class="form-horizontal" >
        ...
        <div class="has-feedback col-xs-12 col-sm-8 col-lg-8 col-md-8 col-sm-offset-2 col-md-offset-2">
            <table align="center" class="table table-responsive box" style="border:1px #CCCCCC solid; margin-top:30px; color: #000;">
                            <tr>
                <td  style="background:#5d9ed1" colspan="3" align="center"><span style="font-size:22px; color:#333; display:block; color:#FFFFFF">
Founder Message	</span> </td>
            </tr>
...
Message &nbsp;</td>
    ...
    <tr>
                <td bgcolor="#f4f4f4" style="background:#222d32" colspan="3"align="center"  style="background:#5d9ed1" >
                <?php 
                    if(!$chek_all){?>
