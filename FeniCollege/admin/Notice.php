<?php
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="INSERT INTO `notice`(`Notice_id`,`Date_time`,`Title`,`Extension`,`Description`) VALUES('".$fetch_query[0]."','$date','$title','$file_ext','$detaile')";
...
if(isset($_POST["view"]))
{
    $query="select * from notice ORDER BY `Date_time` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='Notice.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>View Notice </span>"."</td>"."</tr>";
        if($result){
            $table.="<tr>"."<td>Date</td>"."<td>Title</td>"."<td>Notice Details Or File</td>"."<td>Edit Or Delete</td>"."</tr>";
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `notice` WHERE `Notice_id`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='Notice.php'</script>";
    $fetch_query[0]=$db->withoutPrefix('notice','Notice_id',"3".$prefix,'10');
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM `notice` WHERE `Notice_id`='".$_GET["edit"]."'";
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="REPLACE INTO `notice`(`Notice_id`,`Date_time`,`Title`,`Extension`,`Description`) VALUES('".$_POST["id"]."','$date','$title','$file_ext','$detaile')";
				...
				//print "dd";
					...
					$selext = "SELECT `Extension` FROM `notice` WHERE `Notice_id`='".$_POST["id"]."'";
					$resultext =  $db->select_query($selext);
					if($resultext > 0){
						$fetch_ext = $resultext->fetch_array();
					}
					
                                    @$inserquery="REPLACE INTO `notice`(`Notice_id`,`Date_time`,`Title`,`Extension`,`Description`) VALUES('".$_POST["id"]."','$date','$title','$fetch_ext[0]','$detaile')";
                  
                   
                }  
                  @$chek_insert=$db->update_query($inserquery);
				   $select_query="SELECT * FROM `notice` WHERE `Notice_id`='".$_POST["id"]."'";
        $chek_query=$db->select_query($select_query);
        if($chek_query){
            $fetch_query=$chek_query->fetch_array();
        }
