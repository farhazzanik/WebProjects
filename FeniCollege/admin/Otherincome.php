<?php
 error_reporting(1);
	...
	$fetch[0]=$db->autogenerat('other_income','id','OTI-','9');
...
if(isset($_POST['add']))
	{
		...
		{
			$query_ass="INSERT INTO `other_income` (`id`,`date`,`title`,`description`,`amount`,`admin_id`) VALUES ('$id','$date','$title','$description','$amount','$adminame')";
...
if(isset($_POST['Update']))
	{
		...
		if(!empty($id) && !empty($date) && !empty($title) && !empty($amount) && !empty($adminame))
		{
			$query_ass="REPLACE INTO `other_income` (`id`,`date`,`title`,`description`,`amount`,`admin_id`) VALUES ('$id','$date','$title','$description','$amount','$adminame')";
...
//link edit data...................................	
	...
	if(isset($_GET['edit']))
	{
		...
		$query="SELECT * FROM `other_income` WHERE `id`='$src_text'";
...
<link rel="stylesheet" type="text/css" href="textEdit/css/style.css" />
  ...
  <form name="" action="" method="post"  enctype="multipart/form-data" class="form-horizontal" >
  	...
  	<table align="center" class="table table-responsive box" style="border:1px #CCCCCC solid; margin-top:30px; color: #000;">
  			<tr>
  				<td bgcolor="#f4f4f4" class="warning" colspan="4" bgcolor="#dddddd" align="center"><span style="font-size:22px; color:#333; display:block;">Other Income </span> </td>
  			</tr>
			...
			<tr>
				...
				<td class="info">
					...
					<input type="hidden" name="id" value="<?php echo $fetch[0];?>" />
							...
							<input type="text" id="example2"  class="form-control" name="date"<?php if($chek){?> value="<?php echo $fetch[1];?>" <?php } ?> />
			...
			<tr>
				...
				<td class="info">
					<div class="col-lg-12 has-warning">
							...
							<input type="text" placeholder="Title" class="form-control" name="title" <?php if($chek){?> value="<?php echo $fetch[2];?>" <?php } ?> />
						<span class="glyphicon glyphicon-warning-sign form-control-feedback"></span>
					</div>
				</td>
				<td class='info'><span class="text-danger text-justify"><strong></strong></span></td>
