...
if(isset($_POST['add']))
	{
		...
		{
			$query="INSERT INTO `add_subject_part_info` (`part_id`,`class_id`,`exam_type`,`group_id`,`subject_name`,`subject_part_name`,`subject_part_code`)
					VALUES('".$fetch[0]."','$explodeclass_name[0]','$exploide_examtype[0]','$exploide[0]','$explodesubject_name[0]','$subject_part_name','$subjectpart_code')";
...
if(isset($_POST['Update']))
	{
		...
		if(!empty($id) && !empty($classname) && !empty($class_section) && !empty($subject_name) && !empty($subjectpart_code)  && !empty($subject_name))
		{
			$query="REPLACE INTO `add_subject_part_info` (`part_id`,`class_id`,`exam_type`,`group_id`,`subject_name`,`subject_part_name`,`subject_part_code`)
					VALUES('$id','$explodeclass_name[0]','$exploide_examtype[0]','$exploide[0]','$explodesubject_name[0]','$subject_part_name','$subjectpart_code')";
...
//view data...............................	
	if(isset($_POST['View']))
	{
        ...
        $table.="<tr class='warning' >"."<td align='left' colspan='7'>"."<a href='add_Subject_part.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."<span class='text-success' style='font-size:18px; padding-left:380px; font-weight:blod;'>View All Subject Part </span>"."</td>"."</tr>";
				while($fetch_class=$checked_class->fetch_array())
				{
					$table.="<tr class='success'>"."<td align='center' colspan='7'>"."<strong>"."<span class='text-justify text-danger' style='font-size:18px;'>".$fetch_class[1]."</span>"."</strong>"."</td>"."</tr>";
					...
					if($cheked_group)
					{
						...
						{
							$table.="<tr class='warning'>"."<td align='center' colspan='7'>"."<strong>"."<span class='text-justify text-success' style='font-size:15px'>".$fetch_group[2]."</span>"."</strong>"."</td>"."</tr>";

							$select_all="SELECT `add_subject_part_info`.*,`add_subject_info`.`subject_name`,`subject_code`,`select_subject_type`,`exam_type_info`.`exam_type` FROM `add_subject_part_info`
...
WHERE `add_subject_part_info`.`class_id`='".$fetch_class[0]."' AND `add_subject_part_info`.`group_id`='".$fetch_group[1]."' ORDER BY `add_subject_info`.`serial` ASC ";
							...
							if($chekced_all)
							{
								$table.="<tr class=''>"."<td>Main Subject Name</td>"."<td> Main Subject code</td>"."<td>Subject Part Name</td>"."<td>Subject Part Code</td>"."<td>Subject Type</td>"."<td>Exam Type</td>"."<td>Edit </td>"."</tr>";
								while($fetch_all=$chekced_all->fetch_array())
								{
									$count=mysqli_num_fields($chekced_all);
									
									$table.="<tr>"."<td>".$fetch_all[7]."</td>"."<td>".$fetch_all[8]."</td>"."<td>".$fetch_all[5]."</td>"."<td>".$fetch_all[6]."</td>"."<td>".$fetch_all[9]."</td>"."<td>".$fetch_all[10]."</td>";
								
									$table.="<td  align='center'>";
									$table.="<a href='?edit=$fetch_all[0]&cs=$fetch_all[1]&gp=$fetch_all[3]' class='btn btn-primary' style='width:80px' confirm_click='return confirm_click()'>Edit</a>";
									$table.="</td>";
									$table.="</tr>";
								}
							}
...
//link edit data...................................	
	...
	if(isset($_GET['edit']))
	{
		...
		$query="SELECT `add_subject_part_info`.*,`exam_type_info`.`exam_type`,`add_class`.`class_name`,`add_group`.`group_name`,`add_subject_info`.`subject_name`,`select_subject_type`
FROM `add_subject_part_info` JOIN `exam_type_info` ON `exam_type_info`.`exam_id`=`add_subject_part_info`.`exam_type` JOIN `add_class` ON `add_class`.`id`=`exam_type_info`.`select_class`
JOIN `add_group` ON `add_group`.`class_id`=`add_class`.`id` JOIN `add_subject_info` ON `add_subject_info`.`group_id`=`add_group`.`id`
