	...
	$db = new database();
					...
					if(isset($_POST["submit1"])){	
					  $selectGourpbyId="SELECT `struff_present`.*,`stuff_information`.`stuff_name`,`designation` FROM `struff_present`
					INNER JOIN  `stuff_information` ON `stuff_information`.`stuff_id`=`struff_present`.`StruffID`
					...
					$result_rows_by_teacherID=$db->select_query($selectGourpbyId);
				 	 ...
				 	 $select_groupByDAte="SELECT `struff_present`.*,`stuff_information`.`stuff_name`,`designation` FROM `struff_present`
					INNER JOIN  `stuff_information` ON `stuff_information`.`stuff_id`=`struff_present`.`StruffID`
				...
				<?php 	
					  $selectQuery="SELECT `struff_present`.*,`stuff_information`.`stuff_name`,`designation` FROM `struff_present`
			INNER JOIN  `stuff_information` ON `stuff_information`.`stuff_id`=`struff_present`.`StruffID`
			WHERE `struff_present`.`date` BETWEEN '".$_POST["FristDAte"]."' AND '".$_POST["sndDAte"]."'   GROUP BY `struff_present`.`StruffID`   ORDER BY `struff_present`.`slNo` ASC";
		  ...
		  ?>
		  	<tr>
					   ...
					   <?php  $select_groupByDAte="SELECT `struff_present`.*,`stuff_information`.`stuff_name`,`designation` FROM `struff_present`
			INNER JOIN  `stuff_information` ON `stuff_information`.`stuff_id`=`struff_present`.`StruffID`
			WHERE `struff_present`.`date` BETWEEN '".$_POST["FristDAte"]."' AND '".$_POST["sndDAte"]."'   GROUP BY `struff_present`.`date`   ORDER BY `struff_present`.`slNo` ASC";
		  ...
		  <?php
					...
					?>
					  ...
					  <tr>
					    <td align="left">&nbsp;&nbsp;&nbsp;&nbsp;<strong><?php echo $fetchGroupbyTeacherID["stuff_name"];?></strong></td>
