...
if(isset($_POST["autoID"])){
	...
	if(isset($_POST["forAddFee"])){
			...
			$o = '';
			  $sql ="SELECT * FROM `running_student_info` WHERE `class_id`='$explodeclass[0]' AND `group_id`='$explodegroup[0]' AND  `student_id` LIKE '%".$_POST["forFestdid"]."%'  GROUP BY `student_id`";
			$result = $db->select_query($sql);
			$o = '<ul class="list-unstyled">';
			if($result){
			if(mysqli_num_rows($result)>0){
	...
	if(isset($_POST["forDiscount"])){
			...
			$explodegroup=explode('and',$_POST["groupID"]);
			$o = '';
			   $sql ="SELECT * FROM `student_account_info` WHERE `class_id`='$explodeclass[0]' AND `groupID`='$explodegroup[0]' AND  `studentID` LIKE '%".$_POST["fordiscountID"]."%'  GROUP BY `studentID`";
			$result = $db->select_query($sql);
			$o = '<ul class="list-unstyled">';
			if($result){
			if(mysqli_num_rows($result)>0){
	...
	$o = '';
		...
		if(isset($_POST["forTeacherPaybalSeettion"])){
		
			$o = '';
			$sql ="SELECT * FROM `teacher_payable_master_table` WHERE `teacher_id` LIKE '%".$_POST["teacherID"]."%' GROUP BY `teacher_id`";
			$result = $db->select_query($sql);
			$o = '<ul class="list-unstyled">';
			if($result){
			if(mysqli_num_rows($result)>0){
