...
`add_group`.`id`=`running_student_info`.`group_id` INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`running_student_info`.`student_id` WHERE `running_student_info`.`student_id`='".$_POST["stID"]."'";
	...
	<?php /*$sql3="SELECT `student_academic_record`.*,`student_personal_info`.`addmission_date`,`student_name`,`father_name`,`mother_name`,`gender`,`contact_no`,`email`,
 `add_class`.`class_name`,`add_group`.`group_name` FROM `student_academic_record` JOIN `student_personal_info`
 ON `student_personal_info`.`id`=`student_academic_record`.`student_id` JOIN `add_class` ON `add_class`.`id`=`student_academic_record`.`class_id`
 JOIN `add_group` ON `add_group`.`id`=`student_academic_record`.`groupID` WHERE `student_academic_record`.`student_id`='$id'";
 	$chke3=$db->select_query($sql3);
 if($chke3){
 	$outp = "";
	$fetch3=$chke3->fetch_object();
	$outp ='<table class="table table-responsive table-hover table-bordered" style="margin-top:20px">';
	$outp.='<tr>'.'<td colspan="4">'.'<input type="button" class="btn btn-sm btn-danger" onclick="return ShowAllStde()" value ="BACK"/>';
	$outp.='<span class="text-justify text-success text-left" style="font-size:16px; padding-left:20px"><strong>'.'Class Name &nbsp;:'.$fetch3->class_name.'</strong></span> <span class="text-justify text-success text-right" style=" padding-left:50px;"><strong>Group Name &nbsp; :'.$fetch3->group_name.'</strong></span></td></tr>';
	$outp.='<tr>'.'<Td colspan="4" align="center"><span class="text-justify text-danger "><strong>Session &nbsp; :'.$fetch3->session.'</strong></span><br/>'.'<span class="text-justify text-danger "><strong> Privious Student Roll&nbsp; :'.$fetch3->class_roll.'</strong></span><br/>'.'<span class="text-justify text-danger "><strong>&nbsp;Privous Registration Year&nbsp;:'.$fetch3->year.'&nbsp;</strong></span><br/>
										</Td>
									</tr>';
	$outp.='<tr align="center"><td>Student Name </td><td>:</td><td>'.$fetch3->student_name.'</td><td rowspan="4"><img src="../other_img/'.$fetch3->student_id.'.jpg" alt="'.$fetch3->student_name.'" style="height:180px; width:180px;" class="img-responsive img-thumbnail"/></td></tr>';
	$outp.='<tr align="center"><td>Admission Date</td><td>: </td><td>'.$fetch3->addmission_date.'</td></tr>';
	$outp.='<tr align="center"><td>Father  Name  </td><td>:</td><td>'.$fetch3->father_name.'</td></tr>';
	$outp.='<tr align="center"><td>Mother  Name : </td><td>:</td><td>'.$fetch3->mother_name.'</td></tr>';
	$outp.='<tr align="center"><td>Gender </td><td>:</td><td colspan="2">'.$fetch3->gender.'</td></tr>';
	$outp.='<tr align="center"><td>Contact No </td><td>:</td><td colspan="2">'.$fetch3->contact_no.'</td></tr>';
	$outp.='<tr align="center"><td>Email </tD><td>:</td><td colspan="2">'.$fetch3->email.'</td></tr>';
	$outp.='</table>';
	echo $outp;
 }*/ ?>
  <style>
 ...
 <table width="927" height="193" border="1" cellpadding="0" cellspacing="0" align="center" style="margin-top:10px;">
   <tr class="noneBtnForprin">
    ...
    <td height="33" colspan="9" align="left">&nbsp;
	<input type="button" class="btn btn-sm btn-danger" onclick="return ShowAllStde()" value ="BACK"/>	</td>
   </tr>
  <tr >
  ...
  <tr align="">
	...
	!="null" ) {?><span  style="padding-left:200px;">Group &nbsp; :&nbsp; <?php if(isset($fetchpreRecord)) { 
  ...
  <tr align="center">
    ...
    <td> <a href="#" onclick="return ShowRunnDeatisl('<?php echo $fetchpreRecord["student_id"]?>')"><?php if(isset($fetchpreRecord)) { 
...
ON `add_group`.`id`=`student_academic_record`.`groupID` WHERE `student_academic_record`.`student_id`='".$_POST["stID"]."'";
   ...
   ?>		
	...
	<?php 
												$sql333="SELECT COUNT(`subject_id`) FROM `subject_registration_table` WHERE `std_id`='".$_POST["stID"]."'";
...
<?php
		if(isset($_POST["ViewssubID"])){
		
		$presentrecord = "SELECT `running_student_info`.*,`add_class`.`class_name`,`add_group`.`group_name`,`student_personal_info`.`father_name`,`student_name` FROM `running_student_info`
INNER JOIN `add_class` ON `add_class`.`id`=`running_student_info`.`class_id` INNER JOIN `add_group` ON
`add_group`.`id`=`running_student_info`.`group_id` INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`running_student_info`.`student_id` WHERE `running_student_info`.`student_id`='".$_POST["getid"]."'";
	$resultPreRecord =  $db->select_query($presentrecord);
		if($resultPreRecord->num_rows > 0){
			$fetchpreRecord = $resultPreRecord->fetch_array();
		}
...
INNER JOIN `student_acadamic_information` ON `student_acadamic_information`.`id`=`running_student_info`.`student_id`
 WHERE `running_student_info`.`student_id`='".$_POST["getid"]."'";
...
<?php 	}?>	</td>
 ...
 <tr >
    ...
    <td width="190" height="18" align="left" style="padding-left:20px; font-size:14px;" colspan="4">Registered Subjects : </td>
			...
			$sql8="SELECT `running_student_info`.`student_id`,`class_roll`,`datev`,`year`,`student_personal_info`.`student_name`,`add_class`.`class_name`,`add_group`.`group_name`
 FROM `running_student_info` JOIN `student_personal_info` ON `student_personal_info`.`id`=`running_student_info`.`student_id` JOIN `add_class`
 ON `add_class`.`id`=`running_student_info`.`class_id` JOIN `add_group` ON `add_group`.`id`=`running_student_info`.`group_id` WHERE `running_student_info`.`student_id`='".$_POST["getid"]."'";
 			$chek8=$db->select_query($sql8);
			if($chek8){
				$res8=$chek8->fetch_object();
				//print_r($res8);
