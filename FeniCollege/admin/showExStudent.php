	...
	if(isset($_POST["saveExStudent"])){
		...
		//	 $count = count($_POST["fromData"]);
			  for($a = 0; $a < count($_POST["fromData"]);$a++){
			  			   $select_query = "SELECT `running_student_info`.`student_id`,`class_id`,`group_id`,`class_roll`,`student_acadamic_information`.`session2`,`student_personal_info`.`student_name`,`father_name`,`mother_name`,`gender`
						...
						$resultQuery = $db->select_query($select_query);
							if($resultQuery->num_rows > 0 ){
								$fetchQuery = $resultQuery->fetch_array();
									 $inserQuery = "INSERT INTO `exstudentreport`(`Id`,`StudentName`,`FatherName`,`MotherName`,`ClassId`,`GroupId`,`classRoll`,`session`,`gender`) VALUES('$fetchQuery[student_id]','$fetchQuery[student_name]','$fetchQuery[father_name]','$fetchQuery[mother_name]','$fetchQuery[class_id]','$fetchQuery[group_id]','$fetchQuery[class_roll]','$fetchQuery[session2]','$fetchQuery[gender]')";
