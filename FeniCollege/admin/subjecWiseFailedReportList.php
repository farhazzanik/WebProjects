            ...
            <div class="col-md-10 col-md-offset-1"  style="border:2px #CCCCCC solid; margin-top:10px;"><br/> <br/>
                ...
                <table cellpadding="0" cellspacing="0"  class="table table-responsive table-hover " style="margin-top:0px;">					
                			<tr style="border:1px #CCCCCC solid">
									<td align="center"><img src="all_image/Logo.jpg" style="height:80px; padding-left:2px; padding-top:2px;"/></td>
										<td align="center">
							...
							<tr class="" style="border:1px #CCCCCC solid">
									<td colspan="3" align="center"><span class="text-justify text-success" style="font-size:14px;"><strong>  Subject Wise Failed  List  </strong></span></td>
							</tr>
							<tr class="" style="border:1px #CCCCCC solid">
									<td colspan="3" align="center"><span  class="text-justify text-warning" style="font-size:14px;"><strong>Hints :Student Name(Roll)(Subject Code)</strong></span> </td>
							</tr>
							<?php
								$sqlSubCount="SELECT * FROM `result`  WHERE `totalFailSub` !='0' AND `classId`='$clId' AND `GroupID`='$gpId' AND `session`='$session' AND `examId`='$examId' GROUP BY `totalFailSub`";
							...
							<tr style="border:1px #CCCCCC solid">
									<td colspan="3">
									<?php 
											$sqlq="SELECT * FROM `result` WHERE `totalFailSub`='$fetchSub[totalFailSub]'  AND `classId`='$fetchSub[classId]' AND `GroupID`='$fetchSub[GroupID]' AND `session`='$fetchSub[session]' AND `examId`='$fetchSub[examId]'";
												//print $sqlq;
									...
									?>
									<strong><span>
									<?php $forName="SELECT `student_personal_info`.`student_name` FROM `student_personal_info` WHERE `id`='$fetch_wise_sub[STD_ID]'";
									$resulName=$db->select_query($forName);if($resulName){$fetch_name=$resulName->fetch_array();}
									
									$forRoll="SELECT `studentRoll` FROM `gnerate_marks` WHERE `studentID`='$fetch_wise_sub[STD_ID]'";
									$resultRoll=$db->select_query($forRoll);
									if($resultRoll){
										$fetch_roll=$resultRoll->fetch_array();
									}
									
									 echo "<span class='label label-danger label-as-badge'> ".$sl.".</span>&nbsp;".$fetch_name["student_name"]."&nbsp(&nbsp;".$fetch_roll["studentRoll"]."&nbsp)&nbsp";?> ( <?php  $subsubCod="SELECT `gnerate_marks`.`subjectID`,`add_subject_info`.`subject_code` FROM `gnerate_marks` 
...
$mmm=0;
  if($resultsubcode){while($fetchsubsubCod= $resultsubcode->fetch_array()){
  $mmm++;
