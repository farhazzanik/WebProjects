...
`permanent_PO`,`present_distric`,`permanent_upazila`,`student_acadamic_information`.`session2`,`add_class`.`class_name`,`add_group`.`group_name` FROM`running_student_info`
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`running_student_info`.`student_id` INNER JOIN 
`student_address_info` ON `student_address_info`.`id`=`running_student_info`.`student_id`
INNER JOIN `student_acadamic_information` ON `student_acadamic_information`.`id`=`running_student_info`.`student_id`
INNER JOIN `add_class` ON  `add_class`.`id`=`running_student_info`.`class_id` INNER JOIN `add_group` ON `add_group`.`id`=`running_student_info`.`group_id`
...
.style17 {font-size: 16px}
	...
	<center>
<table width="1325" height="919" background="all_image/charecter_certificateSDMS2015.jpg" style="  border:0px;" align="center" >
  <tr>
  ...
  <tr>
    <td height="615" colspan="2" valign="top"><table width="100%" height="71%" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td height="34"><span class="style12"></span></td>
      ...
      <tr>
		...
		Address       <span style="padding-left:78px;"></span> :	 <?php echo $fetchForall["permanent_village"];?>, <?php echo $fetchForall["permanent_PO"];?>,   <?php echo $fetchForall["permanent_upazila"];?>,  <?php echo $fetchForall["present_distric"];?>.	<br/><br/>is a regular/irregular student of Joypur Sorojini High School.	<br/><br/>
		<?php if($fetchForall["gender"] == "Male") { ?>He <?php } else { ?> She <?php } ?> has been studying here in Class <?php echo $fetchForall["class_name"];?><?php if($fetchForall["group_name"] != "Null") { ?>
		  ,Group <?php echo $fetchForall["group_name"];?> <?php } ?> .
		According to our admission register, <?php if($fetchForall["gender"] == "Male") { ?>his <?php } else { ?> her <?php } ?> date of birth is <?php echo $fetchForall["date_of_brith"];?> 
		<?php if($fetchForall["gender"] == "Male") { ?>.His <?php } else { ?> Her <?php } ?> roll no. is <?php echo $fetchForall["class_roll"];?>.<br/>
		<br/>
