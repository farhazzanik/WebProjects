...
use Session;
use Image;
class memberAddmission extends Controller
{
    //
...
function send_msg($api_url,$from_number, $to_numbers, $text)
  {   
    ...
    public function save(Request $request){
     ...
     $autid = $explode[0].'-'.$request->Member.'-'.$request->Reg;
     //$autid=$this->withoutPrefix('memeradd','Addid',$date,'18');
     
      $explodes= explode('-', $request->Todaydate);
      $Todaydate=$explodes[2].'-'.$explodes[1].'-'.$explodes[0]; 
      
    //   $expz= explode('-', $request->Expiration);
    //   $Expzz=$expz[2].'-'.$expz[1].'-'.$expz[0];
 ...
 public function showAreforMemAdd(Request $request,$id1){


       if($request->ajax()){
            $results  = DB::table('employeecollarea')
                ->join('areainfo','areainfo.id','=','employeecollarea.fk_area_id')
                 ->select('areainfo.area_name','employeecollarea.fk_area_id')
                 ->where('employeecollarea.fk_emp_id',$id1)
                 ->get();
              return $results;
